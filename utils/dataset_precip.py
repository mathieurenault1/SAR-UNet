...
class precipitation_maps_h5(Dataset):
    def __init__(self, folder, in_channels, out_channels, train=True, transform=None):
        ...
        self.num_input = in_channels
        self.num_output = out_channels
        self.sequence_length = in_channels+out_channels

        self.train = train
        # Dataset is all the images
        self.size_dataset = self.n_images-(in_channels+out_channels)
...
class precipitation_maps_oversampled_h5(Dataset):
    def __init__(self, folder, train=True, transform=None, in_channels=12):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            imgs = self.transform(imgs)
        input_img = imgs[:self.in_channels]
        target_img = imgs[-1]
        return input_img, target_img
